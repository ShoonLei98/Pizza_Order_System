...
<div class="container px-4 px-lg-5" id="home">
    ...
    <div class="d-flex">
            ...
            <div class="">
                <div class="py-5 text-center">
                    <form class="d-flex m-5" action="{{ route('user#searchItem') }}" method="get">
                        @csrf
                        <input class="form-control me-2" type="search" name="searchItem" placeholder="Search" aria-label="Search" >
                        <button class="btn btn-outline-dark" type="submit">Search</button>
                    </form>

                    <div class="">
                        <a href="{{ route('user#index') }}" style="text-decoration: none"><div class="m-2 p-2 text-black">All</div></a>
                        @foreach ($category as $item)
                        <a href="{{ route('user#categorySearch', $item->category_id) }}" class="text-black" style="text-decoration: none">
                    ...
                    <form action="{{ route('user#searchPizzaItem') }}" method="get">
                        ...
                        <button type="submit" name="searchPizza" class="btn bg-dark text-white">Search <i class="fas fa-search "></i></button>
                    </form>
                </div>
            </div>
        </div>
        <div class="mt-5">
                ...
                <div class="col-md-4 mb-5">
                    <div class="card h-100" style="width: 300px">
                       @if ($item->buy_one_get_one_status == 1)
                        <!-- Sale badge-->
                        <div class="badge bg-dark text-white position-absolute" style="top: 0.5rem; right: 0.5rem">Buy One Get One</div>
                       @endif
                        <!-- Product image-->
                        <img class="card-img-top" id="pizza-image" src="{{ asset('uploads/'.$item->image) }}" alt="..." height="180px" width="100%"/>
                        <!-- Product details-->
            ...
            @else
            <div class="alert alert-warning" role="alert" style="width: 500px; margin-top: 50px">
                There is no pizza!!!
              </div>
            @endif
            <div>{{ $pizza->links() }}</div>
...
<div class="text-center d-flex justify-content-center align-items-center" id="contact">
    <div class="col-4 border shadow-sm ps-5 pt-5 pe-5 pb-2 mb-5">
        ...
        <form action="{{ route('user#createContact') }}" class="my-4" method="post">
            @csrf
            <input type="text" name="name" id="" class="form-control my-3" placeholder="Name" value="{{ old('name') }}">
            @if ($errors->has('name'))
                <p class="text-danger">{{ $errors->first('name') }}</p>
            @endif

            <input type="text" name="email" id="" class="form-control my-3" placeholder="Email" value="{{ old('email') }}">
            @if ($errors->has('email'))
